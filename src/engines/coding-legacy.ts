/**
 * Internal emergency fallback for the old Servus coding role loop.
 *
 * This module is intentionally not imported by the normal coding path. It is
 * loaded only when SERVUS_INTERNAL_CODING_LEGACY=1 is set for debugging.
 */
...
export class LegacyCodingEngine implements Engine {
  ...
  private async createAgents(ctx: EngineContext): Promise<void> {
    ...
    const opts = { cwd };
    log.info(`Backend: Servus Runtime`);
