 ...
 * dies, the supervisor performs best-effort cleanup (Chrome kill or LocalProvider
...
// Non-SEA: execute BrowserCore CLI entrypoint with supervisor args.
  ...
  if (!resolved) {
    opts?.onError?.(
      new ShutdownSupervisorResolveError(
        "Shutdown supervisor entry missing (expected BrowserCore CLI entrypoint).",
      ),
      "resolve",
    );
    return null;
