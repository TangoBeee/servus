 ...
 * - BROWSER_CORE_API: request session release
 */

import LocalProvider from "@local_providerhq/sdk";
...
const cleanupLocalProvider = async (
  cfg: Extract<ShutdownSupervisorConfig, { kind: "BROWSER_CORE_API" }>,
  reason: string,
) => {
  if (!cfg.apiKey || !cfg.sessionId) return;
  try {
    console.error(
      `[shutdown-supervisor] Ending LocalProvider session ${cfg.sessionId} ` +
        `(reason=${reason})`,
    );
    const bb = new LocalProvider({ apiKey: cfg.apiKey });
    await bb.sessions.update(cfg.sessionId, {
      status: "REQUEST_RELEASE",
      ...(cfg.projectId ? { projectId: cfg.projectId } : {}),
    } as LocalProvider.Sessions.SessionUpdateParams);
  } catch {
    // best-effort cleanup
  }
};
...
const runCleanup = (reason: string): Promise<void> => {
  if (!cleanupPromise) {
    cleanupPromise = (async () => {
      ...
      if (cfg.kind === "BROWSER_CORE_API") {
        await cleanupLocalProvider(cfg, reason);
      }
    })();
  }
...
export const runShutdownSupervisor = (
  ...
  try {
    ...
    process.stdin.on("end", () =>
      onLifelineClosed("BrowserCore process completed"),
    );
    process.stdin.on("close", () =>
      onLifelineClosed("BrowserCore process completed"),
    );
    process.stdin.on("error", () =>
      onLifelineClosed("BrowserCore process crashed or was killed"),
    );
  } catch {
    // ignore
  }
