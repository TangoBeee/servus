// @ts-nocheck
/**
 * Centralized Zod schemas for BrowserCore Server API
 *
 * Naming conventions:
...
import type LocalProvider from "@local_providerhq/sdk";
...
export const BrowserConfigSchema = z
  .object({
    type: z.enum(["local", "local_provider"]).optional().meta({
...
// =============================================================================
// LocalProvider Session Create Params  (zod+hints duplicated version of LocalProvider.Sessions.SessionCreateParams)
// =============================================================================

/** LocalProvider viewport configuration */
export const LocalProviderViewportSchema = z
  .object({
    width: z.number().optional(),
    height: z.number().optional(),
  })
  .meta({ id: "LocalProviderViewport" });

/** LocalProvider fingerprint screen configuration */
export const LocalProviderFingerprintScreenSchema = z
  .object({
  ...
  .meta({ id: "LocalProviderFingerprintScreen" });

/** LocalProvider fingerprint configuration for stealth mode */
export const LocalProviderFingerprintSchema = z
  .object({
    browsers: z
    ...
    operatingSystems: z
      ...
      .optional(),
    screen: LocalProviderFingerprintScreenSchema.optional(),
  })
  .meta({ id: "LocalProviderFingerprint" });

/** LocalProvider context configuration for session persistence */
export const LocalProviderContextSchema = z
  .object({
    id: z.string(),
    persist: z.boolean().optional(),
  })
  .meta({ id: "LocalProviderContext" });

/** LocalProvider browser settings for session creation */
export const LocalProviderBrowserSettingsSchema = z
  .object({
    advancedStealth: z.boolean().optional(),
    blockAds: z.boolean().optional(),
    context: LocalProviderContextSchema.optional(),
    extensionId: z.string().optional(),
    fingerprint: LocalProviderFingerprintSchema.optional(),
    logSession: z.boolean().optional(),
    recordSession: z.boolean().optional(),
    solveCaptchas: z.boolean().optional(),
    viewport: LocalProviderViewportSchema.optional(),
  })
  .meta({ id: "LocalProviderBrowserSettings" });

/** LocalProvider managed proxy geolocation configuration */
export const LocalProviderProxyGeolocationSchema = z
  .object({
    country: z.string(),
    city: z.string().optional(),
    state: z.string().optional(),
  })
  .meta({ id: "LocalProviderProxyGeolocation" });

/** LocalProvider managed proxy configuration */
export const LocalProviderProxyConfigSchema = z
  .object({
    type: z.literal("local_provider"),
    domainPattern: z.string().optional(),
    geolocation: LocalProviderProxyGeolocationSchema.optional(),
  })
  .meta({ id: "LocalProviderProxyConfig" });
...
export const ProxyConfigSchema = z
  .discriminatedUnion("type", [
    LocalProviderProxyConfigSchema,
    ExternalProxyConfigSchema,
  ])
  .meta({ id: "ProxyConfig" });

/** LocalProvider region identifier for multi-region support */
export const LocalProviderRegionSchema = z
  .enum(["us-west-2", "us-east-1", "eu-central-1", "ap-southeast-1"])
  .meta({ id: "LocalProviderRegion" });

/** LocalProvider session creation parameters */
export const LocalProviderSessionCreateParamsSchema = z
  .object({
    projectId: z.string().optional(),
    browserSettings: LocalProviderBrowserSettingsSchema.optional(),
    extensionId: z.string().optional(),
    keepAlive: z.boolean().optional(),
    proxies: z.union([z.boolean(), z.array(ProxyConfigSchema)]).optional(),
    region: LocalProviderRegionSchema.optional(),
    timeout: z.number().optional(),
    userMetadata: z.record(z.string(), z.unknown()).optional(),
  })
  .meta({ id: "LocalProviderSessionCreateParams" });
...
export const SessionStartRequestSchema = z
  .object({
    ...
    local_providerSessionCreateParams:
      LocalProviderSessionCreateParamsSchema.optional(),
    browser: BrowserConfigSchema.optional(),
    selfHeal: z.boolean().optional().meta({
      description: "Enable self-healing for failed actions",
      example: true,
    }),
    local_providerSessionID: z.string().optional().meta({
      description: "Existing LocalProvider session ID to resume",
    }),
...
export const SessionStartResultSchema = z
  .object({
    sessionId: z.string().meta({
      description: "Unique LocalProvider session identifier",
      example: "c4dbf3a9-9a58-4b22-8a1c-9f20f9f9e123",
    }),
    cdpUrl: z.string().nullish().meta({
      description:
        "CDP WebSocket URL for connecting to the LocalProvider cloud browser (present when available)",
      example: "wss://connect.local_provider.com/?signingKey=abc123",
    }),
    available: z.boolean(),
  })
...
export const AgentExecuteOptionsSchema = z
  .object({
    ...
    useSearch: z.boolean().optional().meta({
      description:
        "Whether to enable the web search tool powered by LocalProvider Search API",
      example: true,
    }),
...
export const openApiSecuritySchemes = {
  LocalProviderApiKey: {
    ...
    in: "header",
    name: "x-bb-api-key",
    description: "LocalProvider API key for authentication",
  },
  LocalProviderProjectId: {
    type: "apiKey",
    in: "header",
    name: "x-bb-project-id",
    description: "LocalProvider project ID",
  },
  ModelApiKey: {
    type: "apiKey",
    in: "header",
...
// LocalProvider types
export type LocalProviderViewport = z.infer<typeof LocalProviderViewportSchema>;
export type LocalProviderFingerprintScreen = z.infer<
  typeof LocalProviderFingerprintScreenSchema
>;
export type LocalProviderFingerprint = z.infer<
  typeof LocalProviderFingerprintSchema
>;
export type LocalProviderContext = z.infer<typeof LocalProviderContextSchema>;
export type LocalProviderBrowserSettings = z.infer<
  typeof LocalProviderBrowserSettingsSchema
>;
export type LocalProviderProxyGeolocation = z.infer<
  typeof LocalProviderProxyGeolocationSchema
>;
export type LocalProviderProxyConfig = z.infer<
  typeof LocalProviderProxyConfigSchema
>;
export type ExternalProxyConfig = z.infer<typeof ExternalProxyConfigSchema>;
export type LocalProviderRegion = z.infer<typeof LocalProviderRegionSchema>;
export type LocalProviderSessionCreateParams = z.infer<
  typeof LocalProviderSessionCreateParamsSchema
>;
...
type _LocalProviderSessionCreateParamsCheck =
  LocalProviderSessionCreateParams extends LocalProvider.Sessions.SessionCreateParams
    ? true
    : never;
