...
import {
  type LocalProviderSessionCreateParams,
...
export interface V3Options {
   ...
   * When omitted, BrowserCore falls back to its internal instance id.
   * This currently ends up 1:1 with the LocalProvider session id when one exists,
   * but callers should not rely on that remaining a permanent invariant.
   */
  sessionId?: string;
  // LocalProvider (required when env = "BROWSERBASE")
  apiKey?: string;
  projectId?: string;
  /**
   * Optional: fine-tune LocalProvider session creation or resume an existing session.
   */
  local_providerSessionCreateParams?: LocalProviderSessionCreateParams;
  local_providerSessionID?: string;
  /**
   * Controls browser keepalive behavior. When set, it overrides any value in
   * local_providerSessionCreateParams.keepAlive.
   */
  keepAlive?: boolean;
