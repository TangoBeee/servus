...
import { BrowserCoreLogger, LoggerOptions } from "../logger.js";
...
// No captcha solver
// Pruned dangling import: ./launch/local_provider.js
// Pruned dangling import: ./launch/local.js
...
// Pruned dangling import: ./mcp/utils.js
import {
  ActHandlerParams,
...
import {
  ...
  BrowserCoreInvalidArgumentError,
  BrowserCoreNotInitializedError,
...
// Pruned dangling import: ./api.js
// No experimental features or variable formatting

// Pruned dangling import: ./flowlogger/EventEmitter.js
// Pruned dangling import: ./flowlogger/EventStore.js
...
function resolveModelConfiguration(
  ...
  if (model && typeof model === "object") {
    ...
    if (!modelName) {
      throw new BrowserCoreInvalidArgumentError(
 ...
 *
 * Purpose:
 * A high-level orchestrator for BrowserCore V3. Abstracts away whether the browser
 * runs **locally via Chrome** or remotely on **LocalProvider**, and exposes simple
 * entrypoints (`act`, `extract`, `observe`) that delegate to the corresponding
 * handler classes.
 *
 * Responsibilities:
 * - Bootstraps Chrome or LocalProvider, ensures a working CDP WebSocket, and builds a `V3Context`.
...
export class V3 {
  ...
  public local_providerSessionId?: string;
  private local_providerSessionUrl?: string;
  private local_providerDebugUrl?: string;
  public get local_providerSessionID(): string | undefined {
    return this.local_providerSessionId;
  }
  public get local_providerSessionURL(): string | undefined {
    return this.local_providerSessionUrl;
  }
  public get local_providerDebugURL(): string | undefined {
    return this.local_providerDebugUrl;
  }
  /**
   * Returns true if the browser is running on LocalProvider.
   */
  public get isLocalProvider(): boolean {
    return this.state.kind === "BROWSERBASE";
  }

  /**
   * Returns true if captcha auto-solving is enabled on LocalProvider.
   * Defaults to true when not explicitly set to false.
   */
  public get isCaptchaAutoSolveEnabled(): boolean {
    return (
      this.isLocalProvider &&
      this.opts.local_providerSessionCreateParams?.browserSettings
        ?.solveCaptchas !== false
    );
  }

  /**
   * Returns true if advancedStealth is enabled in LocalProvider settings.
   */
  ...
  public get configuredViewport(): { width: number; height: number } {
    ...
    if (this.opts.env === "BROWSERBASE") {
      const vp =
        this.opts.local_providerSessionCreateParams?.browserSettings?.viewport;
      return {
  ...
  private _onCdpClosed = (why: string) => {
    if (this.state.kind === "BROWSERBASE") {
      void this._logLocalProviderSessionStatus();
    }
  ...
  private browser_coreLogger: BrowserCoreLogger;
  ...
  private apiClient: BrowserCoreAPIClient | null = null;
  private keepAlive?: boolean;
  private shutdownSupervisor: ShutdownSupervisorHandle | null = null;

  public browser_coreMetrics: BrowserCoreMetrics = {
    actPromptTokens: 0,
  ...
  constructor(opts: V3Options) {
    ...
    this.keepAlive =
      opts.keepAlive ?? opts.local_providerSessionCreateParams?.keepAlive;

    // Create per-instance BrowserCoreLogger (handles usePino, verbose, externalLogger)
    // This gives each V3 instance independent logger configuration
    // while still sharing the underlying Pino worker thread via BrowserCoreLogger.sharedPinoLogger
    ...
    this.browser_coreLogger = new BrowserCoreLogger(loggerOptions, opts.logger);
    this.browser_coreLogger.setVerbosity(this.verbose);
    ...
    try {
      if (this.externalLogger) {
        ...
        // Fall back to browser_coreLogger when no external logger
        bindInstanceLogger(this.instanceId, (line) => {
          this.browser_coreLogger.log(line);
        });
      }
    } catch {
      // ignore
   ...
   */
  public get metrics(): Promise<BrowserCoreMetrics> {
    if (this.apiClient) {
      ...
      return this.apiClient.getReplayMetrics().catch((error) => {
        ...
        return this.browser_coreMetrics;
      });
    }
    // Return local metrics wrapped in a Promise for consistency
    return Promise.resolve(this.browser_coreMetrics);
  }
  ...
  public updateMetrics(
    ...
    switch (functionName) {
      case V3FunctionName.ACT:
        this.browser_coreMetrics.actPromptTokens += promptTokens;
        this.browser_coreMetrics.actCompletionTokens += completionTokens;
        this.browser_coreMetrics.actReasoningTokens += reasoningTokens;
        this.browser_coreMetrics.actCachedInputTokens += cachedInputTokens;
        this.browser_coreMetrics.actInferenceTimeMs += inferenceTimeMs;
        break;

      case V3FunctionName.EXTRACT:
        this.browser_coreMetrics.extractPromptTokens += promptTokens;
        this.browser_coreMetrics.extractCompletionTokens += completionTokens;
        this.browser_coreMetrics.extractReasoningTokens += reasoningTokens;
        this.browser_coreMetrics.extractCachedInputTokens += cachedInputTokens;
        this.browser_coreMetrics.extractInferenceTimeMs += inferenceTimeMs;
        break;

      case V3FunctionName.OBSERVE:
        this.browser_coreMetrics.observePromptTokens += promptTokens;
        this.browser_coreMetrics.observeCompletionTokens += completionTokens;
        this.browser_coreMetrics.observeReasoningTokens += reasoningTokens;
        this.browser_coreMetrics.observeCachedInputTokens += cachedInputTokens;
        this.browser_coreMetrics.observeInferenceTimeMs += inferenceTimeMs;
        break;

      case V3FunctionName.AGENT:
        this.browser_coreMetrics.agentPromptTokens += promptTokens;
        this.browser_coreMetrics.agentCompletionTokens += completionTokens;
        this.browser_coreMetrics.agentReasoningTokens += reasoningTokens;
        this.browser_coreMetrics.agentCachedInputTokens += cachedInputTokens;
        this.browser_coreMetrics.agentInferenceTimeMs += inferenceTimeMs;
        break;
    }
  ...
  private updateTotalMetrics(
    ...
    this.browser_coreMetrics.totalPromptTokens += promptTokens;
    this.browser_coreMetrics.totalCompletionTokens += completionTokens;
    this.browser_coreMetrics.totalReasoningTokens += reasoningTokens;
    this.browser_coreMetrics.totalCachedInputTokens += cachedInputTokens;
    this.browser_coreMetrics.totalInferenceTimeMs += inferenceTimeMs;
  }
        ...
        try {
          this.logger({
            ...
            message:
              ...
              "If this process exits unexpectedly, local Chrome or LocalProvider " +
  ...
  /**
   * Entrypoint: initializes handlers, launches Chrome or LocalProvider,
      ...
      return await withInstanceLogContext(this.instanceId, async () => {
        ...
        if (this.opts.env === "LOCAL") {
          ...
          if (lbo.cdpUrl) {
            ...
            this.resetLocalProviderSessionMetadata();
          ...
          if (!userDataDir) {
            const base = path.join(os.tmpdir(), "browser_core-v3");
        ...
        if (this.opts.env === "BROWSERBASE") {
          const { apiKey, projectId } = this.requireLocalProviderCreds();
          this.logger({
            category: "init",
            message: "Starting local_provider session",
            level: 1,
          });
          const baseSessionParams =
            this.opts.local_providerSessionCreateParams ?? {};
          ...
          if (!this.disableAPI && !this.experimental) {
            this.apiClient = new BrowserCoreAPIClient({
              apiKey,
              projectId,
            ...
            const { sessionId, available } = await this.apiClient.init({
              ...
              local_providerSessionCreateParams: createSessionPayload,
              local_providerSessionID: this.opts.local_providerSessionID,
            });
            if (!available) {
              this.apiClient = null;
            }
            this.opts.local_providerSessionID = sessionId;
          }
          const { ws, sessionId, bb } = await createLocalProviderSession(
            apiKey,
            projectId,
            effectiveSessionParams,
            this.opts.local_providerSessionID,
          );
          this.ctx = await V3Context.create(ws, {
            env: "BROWSERBASE",
          ...
          this.local_providerSessionId = sessionId;
          if (!keepAlive && !this.disableAPI) {
            this.startShutdownSupervisor({
              kind: "BROWSER_CORE_API",
              sessionId,
              apiKey,
              projectId,
            });
          ...
          await this._ensureLocalProviderDownloadsEnabled();

          const resumed = !!this.opts.local_providerSessionID;
          ...
          const sessionUrl = `https://www.local_provider.com/sessions/${sessionId}`;
          this.local_providerSessionUrl = sessionUrl;
          this.local_providerDebugUrl = debugUrl;
          ...
          try {
            this.logger({
              ...
              message: resumed
                ? this.apiClient
                  ? "LocalProvider session started"
                  : "LocalProvider session resumed"
                : "LocalProvider session started",
              level: 1,
              auxiliary: {
        ...
        throw new BrowserCoreInitError(`Unsupported env: ${neverEnv}`);
      });
    } catch (error) {
  ...
  private async _ensureLocalProviderDownloadsEnabled(): Promise<void> {
  ...
  private resetLocalProviderSessionMetadata(): void {
    this.local_providerSessionId = undefined;
    this.local_providerSessionUrl = undefined;
    this.local_providerDebugUrl = undefined;
  }
  ...
  @FlowLogger.wrapWithLogging({
    eventType: "BrowserCoreAct",
  })
  async act(input: string | Action, options?: ActOptions): Promise<ActResult> {
    return await withInstanceLogContext(this.instanceId, async () => {
      if (!this.actHandler) throw new BrowserCoreNotInitializedError("act()");
  ...
  async extract<T extends BrowserCoreZodSchema>(
    instruction: string,
    schema: T,
    options?: ExtractOptions,
  ): Promise<InferBrowserCoreSchema<T>>;

  @FlowLogger.wrapWithLogging({
    eventType: "BrowserCoreExtract",
  })
  async extract(
    a?: string | ExtractOptions,
    b?: BrowserCoreZodSchema | ExtractOptions,
    c?: ExtractOptions,
  ): Promise<unknown> {
    return await withInstanceLogContext(this.instanceId, async () => {
      if (!this.extractHandler) {
        throw new BrowserCoreNotInitializedError("extract()");
      }

      // Normalize args
      let instruction: string | undefined;
      let schema: BrowserCoreZodSchema | undefined;
      let options: ExtractOptions | undefined;

      if (typeof a === "string") {
        instruction = a;
        const isZodSchema = (val: unknown): val is BrowserCoreZodSchema =>
          !!val &&
          typeof val === "object" &&
          "parse" in val &&
          "safeParse" in val;
        if (isZodSchema(b)) {
          schema = b as BrowserCoreZodSchema;
      ...
      const handlerParams: ExtractHandlerParams<BrowserCoreZodSchema> = {
        instruction,
        schema: effectiveSchema as BrowserCoreZodSchema | undefined,
        model: options?.model,
        timeout: options?.timeout,
      ...
      if (this.apiClient) {
        ...
        result =
          await this.extractHandler.extract<BrowserCoreZodSchema>(handlerParams);
      }
  ...
  async observe(
    ...
    return await withInstanceLogContext(this.instanceId, async () => {
      if (!this.observeHandler) {
        throw new BrowserCoreNotInitializedError("observe()");
      }
  ...
  connectURL(): string {
    if (this.state.kind === "UNINITIALIZED") {
      throw new BrowserCoreNotInitializedError("connectURL()");
    }
  ...
  async close(opts?: { force?: boolean }): Promise<void> {
    ...
    // apiClient.end() can cause the hosted API to terminate the LocalProvider
    ...
    // End LocalProvider session via API when keepAlive is not enabled
  ...
  /**
   * Resolves the LocalProvider API key from options or environment variables.
   * Returns undefined if no key is found (does not throw).
   */
  public get local_providerApiKey(): string | undefined {
    return this.opts.apiKey || process.env.BROWSERBASE_API_KEY;
  }

  /** Guard: ensure LocalProvider credentials exist in options. */
  private requireLocalProviderCreds(): {
    apiKey: string;
    projectId?: string;
  } {
    ...
    this.logger({
      category: "init",
      message: "Using LocalProvider credentials",
      level: 1,
    });
  ...
  public get logger(): (logLine: LogLine) => void {
    // Delegate to per-instance BrowserCoreLogger
    // BrowserCoreLogger handles: verbosity filtering, usePino selection, external logger routing
  ...
  private async resolvePage(page?: AnyPage): Promise<Page> {
    ...
    const ctx = this.ctx;
    if (!ctx) {
      throw new BrowserCoreNotInitializedError("resolvePage()");
    }
  ...
  private async normalizeToV3Page(input: AnyPage): Promise<Page> {
    ...
    if (this.isPlaywrightPage(input)) {
      ...
      if (!page)
        throw new BrowserCoreInitError(
    ...
    throw new BrowserCoreInvalidArgumentError("Unsupported page object.");
  }

  private async _logLocalProviderSessionStatus(): Promise<void> {
    if (this.state.kind !== "BROWSERBASE") {
      return;
    }
    ...
    try {
      ...
      const message =
        snapshot.status === "TIMED_OUT"
          ? `LocalProvider session timed out (sessionId: ${sessionId})`
          : `LocalProvider session status: ${snapshot.status}`;
   ...
   * Mirrors the v2 BrowserCore.agent() tool mode (no CUA provider here).
   *
  ...
  agent(options?: AgentConfig): {
    ...
    if (options?.cua === true) {
      ...
      console.warn(
        '[BrowserCore] DEPRECATED: The "cua: true" option is deprecated. Use "mode: \'cua\'" instead.',
      );
    }
