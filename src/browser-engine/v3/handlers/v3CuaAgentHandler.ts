...
// Pruned dangling import: ../agent/AgentClient.js
// Pruned dangling import: ../agent/AgentProvider.js
// Pruned dangling import: ../agent/GoogleCUAClient.js
// Pruned dangling import: ../agent/OpenAICUAClient.js
// Pruned dangling import: ../agent/utils/cuaKeyMapping.js
// Pruned dangling import: ../agent/utils/xpath.js
// Pruned dangling import: ../types/public/agent.js
import { LogLine } from "../types/public/logs.js";
import { type Action, V3FunctionName } from "../types/public/methods.js";

import { toTitleCase } from "../../utils.js";
import { BrowserCoreClosedError } from "../types/public/sdkErrors.js";
// Pruned dangling import: ../agent/utils/captchaSolver.js
...
export class V3CuaAgentHandler {
   ...
   * Throws BrowserCoreClosedError if browser_core.close() was called.
   */
  private ensureNotClosed(): void {
    if (!this.v3.context) {
      throw new BrowserCoreClosedError();
    }
  }
      ...
      if (this.captchaSolver) {
        if (this.captchaSolver.isSolving()) {
          this.logger({
            category: "agent",
            message:
              "Captcha detected — waiting for LocalProvider to solve it before continuing",
            level: 1,
          });
        }
  ...
  async execute(
    ...
    // Set up captcha solver for LocalProvider environments
    ...
    switch (action.type) {
      case "click": {
        ...
        if (recording) {
          ...
          if (normalized) {
            const browser_coreAction: Action = {
              selector: normalized,
            ...
            this.recordCuaActStep(
              action,
              [browser_coreAction],
              browser_coreAction.description,
            );
          }
        } else {
      ...
      case "keypress": {
        ...
        if (recording && browser_coreActions.length > 0) {
          this.recordCuaActStep(
            action,
            browser_coreActions,
            browser_coreActions
  ...
  private recordCuaActStep(
    agentAction: AgentAction,
    browser_coreActions: Action[],
    fallback: string,
  ): void {
    if (!browser_coreActions.length) return;
    const instruction = this.buildInstructionFallback(agentAction, fallback);
    const description = browser_coreActions[0]?.description || instruction;
    const actions = browser_coreActions.map((act) => ({
      ...act,
