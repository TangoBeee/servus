 ...
 */
export function transformUrlStringsToNumericIds<T extends BrowserCoreZodSchema>(
  schema: T,
...
type ExtractionResponse<T extends BrowserCoreZodObject> = ExtractionResponseBase &
  InferBrowserCoreSchema<T>;
...
export class ExtractHandler {
  ...
  async extract<T extends BrowserCoreZodSchema>(
  ...
  ): Promise<InferBrowserCoreSchema<T> | { pageText: string }> {
    ...
    const baseSchema: BrowserCoreZodSchema = (schema ??
      defaultExtractSchema) as BrowserCoreZodSchema;
    ...
    const objectSchema: BrowserCoreZodObject = isObjectSchema
      ? (baseSchema as BrowserCoreZodObject)
      : (factory.object({
          [WRAP_KEY]: baseSchema as ZodTypeAny,
        }) as BrowserCoreZodObject);

    const [transformedSchema, urlFieldPaths] =
      transformUrlStringsToNumericIds(objectSchema);

    ensureTimeRemaining();
    const extractionResponse: ExtractionResponse<BrowserCoreZodObject> =
      await runExtract<BrowserCoreZodObject>({
        instruction,
        domElements: combinedTree,
        schema: transformedSchema as BrowserCoreZodObject,
        llmClient,
    ...
    let output = rest as InferBrowserCoreSchema<BrowserCoreZodObject>;
    ...
    return output as InferBrowserCoreSchema<T>;
  }
}
