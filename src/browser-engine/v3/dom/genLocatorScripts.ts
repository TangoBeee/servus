...
async function main(): Promise<void> {
  ...
  esbuild.buildSync({
    ...
    platform: "browser",
    target: "es2020",
    globalName: "__browser_coreLocatorScriptsFactory",
    minify: true,
    outfile: bundleOutfile,
  });

  const bundleRaw = fs.readFileSync(bundleOutfile, "utf8").trim();
  const bootstrap = `if (!globalThis.__browser_coreLocatorScripts) { ${bundleRaw}\n  globalThis.__browser_coreLocatorScripts = __browser_coreLocatorScriptsFactory;\n}`;
  ...
  const globalRefs: Record<string, string> = Object.fromEntries(
    sorted.map(([name]) => [
      name,
      `globalThis.__browser_coreLocatorScripts.${name}`,
    ]),
  );
