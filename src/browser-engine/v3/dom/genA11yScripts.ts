...
async function main(): Promise<void> {
  ...
  esbuild.buildSync({
    ...
    platform: "browser",
    target: "es2020",
    globalName: "__browser_coreA11yScriptsFactory",
    minify: true,
    outfile: bundleOut,
  });

  const bundleRaw = fs.readFileSync(bundleOut, "utf8").trim();
  const bootstrap = `if (!globalThis.__browser_coreA11yScripts) { ${bundleRaw}\n  globalThis.__browser_coreA11yScripts = __browser_coreA11yScriptsFactory;\n}`;
  ...
  const globalRefs: Record<string, string> = Object.fromEntries(
    sorted.map(([name]) => [name, `globalThis.__browser_coreA11yScripts.${name}`]),
  );
