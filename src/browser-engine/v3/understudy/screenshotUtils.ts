...
import { BrowserCoreInvalidArgumentError } from "../types/public/sdkErrors.js";
// Pruned dangling import: ../dom/build/screenshotScripts.generated.js
        ...
        .evaluate(
          ({ css, token }) => {
            try {
              ...
              style.setAttribute("data-browser_core-style", token);
            ...
            try {
              ...
              for (const rect of rects) {
                ...
                const root = rect.rootToken
                  ? doc.querySelector(
                      `[data-browser_core-mask-root="${rect.rootToken}"]`,
                    ) || defaultRoot
                  : defaultRoot;
                if (!root) continue;
                if (rect.rootToken) {
                  try {
                    ...
                    if (style && style.position === "static") {
                      ...
                      if (
                        !rootEl.hasAttribute("data-browser_core-mask-root-pos")
                      ) {
                        rootEl.setAttribute(
                          "data-browser_core-mask-root-pos",
                ...
                el.setAttribute("data-browser_core-mask", token);
          ...
          .evaluate(
            ({ token, rootTokens }) => {
              try {
                ...
                if (!doc) return;
                const nodes = doc.querySelectorAll(
                  `[data-browser_core-mask="${token}"]`,
                );
                nodes.forEach((node) => node.remove());
                for (const rootToken of rootTokens) {
                  const root = doc.querySelector(
                    `[data-browser_core-mask-root="${rootToken}"]`,
                  ) as HTMLElement | null;
                  if (!root) continue;
                  ...
                  if (prev !== null) {
                    ...
                    root.removeAttribute("data-browser_core-mask-root-pos");
                  }
                  root.removeAttribute("data-browser_core-mask-root");
                }
              } catch {
                // ignore
              }
