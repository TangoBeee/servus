...
// Pruned dangling import: ../dom/build/scriptV3Content.js
...
const FIRST_TOP_LEVEL_PAGE_TIMEOUT_ENV =
  "BROWSER_CORE_FIRST_TOP_LEVEL_PAGE_TIMEOUT_MS";
...
export class V3Context {
  ...
  public async setExtraHTTPHeaders(
    ...
    if (failures.length) {
      throw new BrowserCoreSetExtraHTTPHeadersError(failures);
    }
  }
  ...
  async awaitActivePage(timeoutMs?: number): Promise<Page> {
    ...
    // If a popup was just triggered, Chrome (especially on LocalProvider)
