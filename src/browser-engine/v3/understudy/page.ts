...
import {
  ...
  BrowserCoreSetExtraHTTPHeadersError,
  BrowserCoreSnapshotError,
} from "../types/public/index.js";
import type { Locator } from "./locator.js";
import {
  BrowserCoreInvalidArgumentError,
  BrowserCoreEvalError,
...
export class Page {
  ...
  private readonly apiClient: BrowserCoreAPIClient | null = null;
  ...
  private constructor(
    ...
    mainFrameId: string,
    apiClient?: BrowserCoreAPIClient | null,
  ...
  public on(event: "console", listener: ConsoleListener): Page {
    if (event !== "console") {
      throw new BrowserCoreInvalidArgumentError(`Unsupported event: ${event}`);
    }
   ...
   * @throws {BrowserCoreSetExtraHTTPHeadersError}
  ...
  async setExtraHTTPHeaders(headers: Record<string, string>): Promise<void> {
    ...
    if (errors.length > 0) {
      throw new BrowserCoreSetExtraHTTPHeadersError(errors);
    }
  }
  ...
  async evaluate<R = unknown, Arg = unknown>(
    ...
    if (exceptionDetails) {
      const msg =
        ...
        "Evaluation failed";
      throw new BrowserCoreEvalError(msg);
    }
  ...
  async snapshot(options?: PageSnapshotOptions): Promise<SnapshotResult> {
    try {
      ...
      throw new BrowserCoreSnapshotError(err);
    }
  }
