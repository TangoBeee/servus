...
// Pruned dangling import: ../dom/build/locatorScripts.generated.js
...
import {
  BrowserCoreElementNotFoundError,
  BrowserCoreInvalidArgumentError,
  BrowserCoreLocatorError,
...
export class Locator {
  ...
  public async setInputFiles(files: SetInputFilesArgument): Promise<void> {
    ...
    try {
      ...
      for (const payload of normalized) {
        ...
        const tmp = path.join(
          os.tmpdir(),
          `browser_core-upload-${Date.now()}-${Math.random().toString(36).slice(2)}${ext}`,
        );
   ...
   *
   * When BrowserCore is driving a browser that cannot see the local filesystem (LocalProvider,
  ...
  async fill(value: string): Promise<void> {
    ...
    try {
      ...
      if (res.exceptionDetails) {
        ...
        throw new BrowserCoreLocatorError("Filling", this.selector, message);
      }
  ...
  public async resolveNode(): Promise<{
    ...
    if (!resolved) {
      throw new BrowserCoreElementNotFoundError([this.selector]);
    }
  ...
  private centerFromBoxContent(content: number[]): { cx: number; cy: number } {
    ...
    if (!content || content.length < 8) {
      throw new BrowserCoreInvalidArgumentError("Invalid box model content quad");
    }
