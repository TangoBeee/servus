...
/**
 * BrowserCoreLogger class that wraps Pino for our specific needs
 *
 * LOGGING PRECEDENCE:
 *
 * Test environments:
 ...
 * This prevents spawning a new worker thread for every BrowserCore instance
 * (which happens when `pino-pretty` transport is used), eliminating the
 * memory/RSS growth observed when many BrowserCore objects are created and
 * disposed within the same process (e.g. a request-per-instance API).
 */
export class BrowserCoreLogger {
  /**
   * Shared Pino logger instance across all BrowserCoreLogger instances.
  ...
  constructor(
    ...
    if (this.usePino) {
      ...
      if (!BrowserCoreLogger.sharedPinoLogger) {
        BrowserCoreLogger.sharedPinoLogger = createLogger(options);
      }
      this.logger = BrowserCoreLogger.sharedPinoLogger;
    }
