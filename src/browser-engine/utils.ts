...
import type { BrowserCoreZodSchema } from "./v3/zodCompat.js";
...
export function getZFactory(schema: BrowserCoreZodSchema): typeof z {
...
function getZ4Def(schema: BrowserCoreZodSchema) {
  return (schema as SchemaInternals)._zod?.def as
    | Record<string, unknown>
    | undefined;
}

function getZ4Bag(schema: BrowserCoreZodSchema) {
  return (schema as SchemaInternals)._zod?.bag as
    | Record<string, unknown>
    | undefined;
}

function getZ3Def(schema: BrowserCoreZodSchema) {
...
function getObjectShape(
  schema: BrowserCoreZodSchema,
): Record<string, BrowserCoreZodSchema> | undefined {
  const z4Shape = getZ4Def(schema)?.shape as
    | Record<string, BrowserCoreZodSchema>
    | undefined;
  if (z4Shape) {
    return z4Shape;
  }
  ...
  if (typeof z3Shape === "function") {
    return (z3Shape as () => Record<string, BrowserCoreZodSchema>)();
  }
...
function getArrayElement(
  schema: BrowserCoreZodSchema,
): BrowserCoreZodSchema | undefined {
...
function getUnionOptions(
  ...
  if (Array.isArray(z4Options)) {
    return z4Options as BrowserCoreZodSchema[];
  }
  const z3Options = getZ3Def(schema)?.options;
  return Array.isArray(z3Options)
    ? (z3Options as BrowserCoreZodSchema[])
    : undefined;
}

function getIntersectionSides(schema: BrowserCoreZodSchema): {
  left?: BrowserCoreZodSchema;
  right?: BrowserCoreZodSchema;
} {
  const z4Def = getZ4Def(schema);
  if (z4Def?.left || z4Def?.right) {
    return {
      left: z4Def?.left as BrowserCoreZodSchema | undefined,
      right: z4Def?.right as BrowserCoreZodSchema | undefined,
    };
  }
  const z3Def = getZ3Def(schema);
  return {
    left: z3Def?.left as BrowserCoreZodSchema | undefined,
    right: z3Def?.right as BrowserCoreZodSchema | undefined,
  };
}

function getEnumValues(schema: BrowserCoreZodSchema): string[] | undefined {
...
function getLiteralValues(schema: BrowserCoreZodSchema): unknown[] {
...
function getStringChecks(schema: BrowserCoreZodSchema): unknown[] {
...
function getStringFormat(schema: BrowserCoreZodSchema): string | undefined {
...
function getPipeEndpoints(schema: BrowserCoreZodSchema): {
  in?: BrowserCoreZodSchema;
  out?: BrowserCoreZodSchema;
} {
  const z4Def = getZ4Def(schema);
  if (z4Def?.in || z4Def?.out) {
    return {
      in: z4Def?.in as BrowserCoreZodSchema | undefined,
      out: z4Def?.out as BrowserCoreZodSchema | undefined,
    };
  }
  return {};
}
...
function getEffectsBaseSchema(
  ...
  return getZ3Def(schema)?.schema as BrowserCoreZodSchema | undefined;
}
...
export function validateZodSchema(schema: BrowserCoreZodSchema, data: unknown) {
...
export function toGeminiSchema(zodSchema: BrowserCoreZodSchema): Schema {
    ...
    case "object": {
      ...
      if (shape) {
        Object.entries(shape).forEach(
          ([key, value]: [string, BrowserCoreZodSchema]) => {
...
export function getZodType(schema: BrowserCoreZodSchema): string {
...
export function transformSchema(
...
): [BrowserCoreZodSchema, ZodPathSegments[]] {
  ...
  if (isKind(schema, "object")) {
    ...
    const newShape: Record<string, BrowserCoreZodSchema> = {};
  ...
  if (isKind(schema, "union")) {
    ...
    const newOptions: BrowserCoreZodSchema[] = [];
    let changed = false;
  ...
  if (isKind(schema, "optional")) {
    ...
    if (inner !== innerType) {
      return [
        (inner as z.ZodTypeAny).optional() as unknown as BrowserCoreZodSchema,
        innerPaths,
      ];
    }
  ...
  if (isKind(schema, "nullable")) {
    ...
    if (inner !== innerType) {
      return [
        (inner as z.ZodTypeAny).nullable() as unknown as BrowserCoreZodSchema,
        innerPaths,
      ];
    }
    return [schema, innerPaths];
...
function isKind(s: BrowserCoreZodSchema, kind: string): boolean {
  try {
...
function makeIdStringSchema(orig: BrowserCoreZodSchema): BrowserCoreZodSchema {
  const userDesc =
